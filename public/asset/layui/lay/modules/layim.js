    ...
    function(i) {
        ...
        var x, b, w, k, C, S = function(i, a, n) {
            ...
            A = function() {
                ...
                //好友分组
                x.find(".layim-list-friend").on("contextmenu", "li",
                    function(a) {
                        var n = e(this),
                            l = '<ul data-index="' + n.find('h5').attr('data-group') + '"><li layim-event="DelFriendGroup">删除分组</li><li layim-event="AddFriendGroup">新增分组</li><li layim-event="EditFriendGroup">重命名分组</li></ul>';
                        n.hasClass("layim-null") || (t.tips(l, this, {
                            tips: 1,
                            time: 0,
                            anim: 5,
            ...
            ti = {
                ...
                about: function() {
                    t.alert("如需商用Layim，请联系官方，此案例只是由于个人爱好，用于demo开发<br>感谢layui公司提供的页面支持", {
                        title: "im-chat",
                        shade: !1
                    })
                },
                ...
                //右击聊天记录
                chatLog1: function(i) {
                    var friend_id= i.parent().attr('data-id');
                    let id = friend_id.substr(12)
                        ,type = i.parent().attr('data-type')
                        ,name = i.parent().attr('data-name');
                    return '/chat_log' ? (t.close(ti.chatLog.index), ti.chatLog.index = t.open({
                        type: 2,
                        maxmin: !0,
                        title: "与 " + name + " 的聊天记录",
                        area: ["450px", "100%"],
                        ...
                        anim: 2,
                        id: "layui-layim-chatlog",
                        content: "/chat_log?id=" + id + "&type=" + type
                    })) : t.msg("未开启更多聊天记录")
                },
                //会话聊天
                ...
                //删除好友分组
                DelFriendGroup:function(i,a){
                    var id= i.parent().attr('data-index');
                    layer.confirm('您确定要删除该好友分组吗？如果分组内还有好友存在，将不允许删除', {
                        btn: ['删除','取消'] //按钮
                    }, function(){
                        e.ajax({
                            url: '/del_friend_group'
                            , type: 'get'
                            , data: {id: id}
                            , success: function (res) {
                                var result = JSON.parse(res);
                                if (result.code === 200) {
                                    e.each(e('.layim-list-friend>li>h5'),function (i,item) {
                                        var index_1=e(item).attr('data-group');
                                        if(index_1 === id){
                                            e(item).parent().remove()
                                        }
                                    });
                ...
                AddFriendGroup:function(i,a){
                    ...
                    layer.open({
                        ...
                        content: '<form class="layui-form" action="" style="margin: 20px">\n' +
                            ...
                            '    <label class="layui-form-label" style="text-align: left;">分组名称：</label>\n' +
                            '    <div class="layui-input-block">\n' +
                            '      <input type="text" name="title" required placeholder="分组名称" id="title" autocomplete="off" class="layui-input">\n' +
                            '    </div>\n' +
                            '  </div>\n' +
                            '</form>',
                        btn:'立即提交',
                        yes:function(index,layero){
                            ...
                            if(title === "" || title === undefined ||title === null){
                                layer.msg('分组名称不能为空', {icon: 5});
                                return;
                            }
                            e.ajax({
                                url: '/add_friend_group'
                                ,type:'get'
                                ,data:{friend_group:title}
                                ,success:function (res) {
                                    var result = JSON.parse(res);
                                    if(result.code === 200){
                                        var str = '<li>\n' +
                                            '<h5 layim-event="spread" data-group="' + result.data.id + '" lay-type="true">\n' +
                                            '  <i class="layui-icon">&#xe61a;</i>' +
                                            '   <span>' + title + '</span>' +
                                            '   <em>(<cite class="layim-count">0</cite>)</em>' +
                                            '</h5>\n' +
                                            '<ul class="layui-layim-list layui-show">\n' +
                                            '  <li class="layui-null" style="    height: 20px;\n' +
                                            '    line-height: 20px;\n' +
                                            '    padding: 0;\n' +
                                            '    font-size: 14px;\n' +
                                            '    color: #999;\n' +
                                            '    text-align: center;\n' +
                                            '    cursor: default;">该分组下暂无好友</li>\n' +
                                            '</ul>\n'+
                                        '</li>';
                                        e('.layim-list-friend>li:last-child').parent().append(str);
                ...
                EditFriendGroup:function(i,a){
                    ...
                    layer.open({
                        ...
                        yes:function(index,layero){
                            ...
                            e.ajax({
                                url: '/edit_friend_group'
                                ,type:'get'
                                ,data:{friend_group:title,id:id}
                                ,success:function (res) {
                                    var result = JSON.parse(res);
                                    if(result.code === 200){
                                        e.each(e('.layim-list-friend>li>h5'),function (i,item) {
                ...
                //删除好友
                menuList: function (i,a) {
                    var id= i.parent().attr('data-id');
                    layer.confirm('您确定要删除该好友吗？删除后将也从对方好友列表中删除您的信息', {
                        btn: ['删除','取消'] //按钮
                    }, function(){
                        let friend_id = id.substr(12);
                        e.ajax({
                            url: '/del_friend'
                            ,type:'get'
                            ...
                            ,success:function (res) {
                                var result = JSON.parse(res);
                                if(result.code === 200){
                                    var index= i.parent().attr('data-index');
                                    e.each(e('.layim-list-friend .layui-layim-list li'),function (i,item) {
                                        var index_1=e(item).attr('data-index'),
                                            id_1=e(item).attr('class').split(' ').shift();
                                        if(index_1 == index && id == id_1){
                                            e(item).remove()
                                        }
                                    });
                                    layer.msg('删除成功', {icon: 1});
                ...
                EditGroup:function (i,a) {
                    ...
                    layer.open({
                        ...
                        content: '<form class="layui-form" action="" style="margin: 20px">\n' +
                            '  <div class="layui-form-item">\n' +
                            '    <label class="layui-form-label" style="text-align: left;">群聊名称：</label>\n' +
                            '    <div class="layui-input-block">\n' +
                            '      <input type="text" name="title" required placeholder="分组名称" id="title" autocomplete="off" class="layui-input">\n' +
                            '    </div>\n' +
                            '  </div>\n' +
                        ...
                        yes:function(index,layero){
                            ...
                            e.ajax({
                                ...
                                ,success:function (res) {
                                    var result = JSON.parse(res);
                                    if(result.code === 200){
                                        e.each(e('.layim-list-group>li'),function (i,item) {
                                            var index_1=e(item).attr('class').substr(11);
                                            if(index_1 == id){
                                                e(item).find('span').first().html(title);
                                            }
                                        });
                                        parent.layer.close(index);
                                        layer.msg('操作成功', {icon: 1});
                ...
                //解散群聊
                DelGroup:function (i,a) {
                    var id= i.parent().attr('data-index');
                    layer.confirm('您确定要解散该群聊吗？', {
                        btn: ['解散','取消'] //按钮
                    }, function(){
                        e.ajax({
                            url: '/del_group'
